from flask import Flask, redirect, render_template, flash, request, redirect
...
def show_playlist(playlist_id):
    ...
    playlists = Playlist.query.all(playlist_id)
    form = PlaylistForm()
...
def show_song(song_id):
    ...
    return render_template("songs.html", song=song, form=form)
...
def add_song():
    ...
    if form.validate_on_submit():
        ...
        song = Song(title=title, artist=artist, length=length)
...
def add_song_to_playlist(playlist_id):
    ...
    if form.validate_on_submit():
          ...
          db.session.commit()
          flash(f"Added song to {playlist.name}")
